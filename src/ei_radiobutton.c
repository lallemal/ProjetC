...
void                    ei_radiobutton_drawfunc                                  (struct	ei_widget_t*	widget,
        ...
        //the rectangle containing the title
        ...
        for (int i = 0; i < to_draw->nb_of_choices; i++) {
                ...
                text_size(&text_rect, &to_draw->tb_color, surface, &allow_rec, to_draw->choices[i]);
        }
...
void                    ei_radiobutton_setdefaults                              (ei_widget_t      *widget){
        ...
        to_configure->title                     = "Radiobutton";
        to_configure->current_button            = NULL;
        to_configure->is_size_set               = EI_FALSE;
...
ei_bool_t process_radio(ei_widget_t* widget, ei_event_t* event, void* user_param)
{
        ...
        //setting the ancient chosen button to the basic color
        ...
        //setting the new selected button in the right color
...
void                    ei_radiobutton_configure                                (ei_widget_t      *widget,
        ...
        if (requested_size){
                to_configure->is_size_set = EI_TRUE;
        }
        ...
        total_height += to_configure->title_height + size_button.height;

        //setting the height to
        if (!to_configure->is_size_set)
